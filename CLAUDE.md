...
- **No file over 300 lines.** 301 is a failure. Build output and the lockfile are
  the only exemptions.
- **Everything in the folder that owns it.** `bin/` entrypoints, `lib/` shell
  libraries, `src/panel/` TypeScript and SCSS, `dist/` build output, `tools/`
  patching, `test/`, `docs/`, `commands/`.
...
Three files currently break the 300-line rule and are listed as debt at the foot
of AGENTS.md. Do not add to them without splitting them first.
