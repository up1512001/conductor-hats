...
workspace. Two chats in one workspace can be on two accounts, answering at the
same time, and the toolbar says which.

Written up as what changed rather than in the order it was found. Several of
these were fixes to earlier fixes in the same release; the failures are kept
where they explain the shape of the answer.

### Added

- **An account per chat, from the toolbar.** Choosing an account writes a pin on
  the chat on screen and leaves the workspace route alone, so the other chats in
  that workspace stay where they are. Setting every chat at once is one line
  under the accounts, shown only when the chat's account and the workspace's
  differ, which is the only time it would do anything.

- **`hats next <profile> [agent]`**, the account for the workspaces created from
  now on. What the New Workspace composer writes. It applies to workspaces that
  appear after it and never to ones that already exist, and stands until another
  is chosen, so one press covers a batch created together.

- **`hats json` reports what a chat is running on**, as `started`, beside what
  its next process will take, as `chat`. `hats json [path] [session]` also takes
  the chat to answer about, which the panel reads out of the window.

- **`hats debug [on|off|status|read|clear]`** records what the panel resolved:
  the scope asked for, the workspace and chat it found and how, the target that
  came back, and what a choice wrote. Off unless turned on, and it records
  decisions rather than anything typed. Diagnosing the panel otherwise means
  injecting a probe, and every injection re-signs the copy and signs it out of
  Conductor. Every diagnosis in this release after it landed was a measurement.
...
- **One toolbar choice moved every chat in the workspace.** It wrote the
  workspace route and then cleared the open chat's pin, so two chats set to two
  accounts converged on whichever was chosen last. That was deliberate in #7,
  which was fixing a label that lied, and it removed the capability along with
  the bug.
...
- **The panel could not tell which workspace was on screen.** It counted
  ...
  fibers held 38 distinct ids and the winner was a workspace that was not open.
  Failing that it matched names, where the only name it could find was the
  repository's, and a choice then bound the whole repository.

  The toolbar button is mounted inside the open chat, so the components
  enclosing it are the answer. Its DOM ancestors carry no React key here, so the
  tree is walked once for the innermost element containing the button and the
  components are climbed from there. The element is remembered, never the fiber:
  React replaces a fiber on every render and leaves the old one holding the props
  it had at the time, and keeping one reported the workspace that was open when
  it was cached for every workspace after it.

- **The panel could not tell which chat was open.** Transcript timestamps were
  the old answer and they fail three ordinary ways: a workspace idle for five
  minutes read as empty, two chats answering within two seconds were refused as
  ambiguous, and a conversation resumed after a compaction is filed under a
  different id. Each left the toolbar with no chat to act on. Conductor's own
  `workspaces.active_session_id` answers instead, filtered to the agent whose
  accounts are on screen so a Codex chat is never offered as the Claude one, and
  `sessions.claude_session_id` wins where the two ids differ because that is the
  one on the command line the router reads. The timestamp scan stays as the
  fallback for a Conductor with no database to read.

- **An account chosen while creating a workspace bound the whole repository.**
  The composer is pressed before the workspace exists, and a binding is one
  `CLAUDE_CONFIG_DIR` in the repository's `.conductor` settings, inherited by
  every workspace under it. Creating one workspace on Work and the next on
  Personal left both on Personal, and moved every other workspace in that
  repository with them. It is recorded and used by the workspaces created after
  it instead.

  Which workspaces those are took three attempts, each corrected against the
  log. Any agent could take it, and Conductor starts one with the working
  directory set to `/` before the workspace's own, which swallowed it.
  Restricting it to real workspaces was still not enough, because a dozen are
  open at any time and each respawns an agent on a resume, a model switch or a
  generator restart. So the workspaces that already exist are written down beside
  the choice and only one absent from that list may use it. The workspace that
  uses it writes itself an ordinary route there and then, which is what stops a
  later choice moving it.

  The panel reports it before anything has run, too. A workspace created a moment
  ago has no route yet, and showing the default until the first message named one
  account while the next spawn used another. Pressing an account in that view
  also left the tick where it was, so it read as a dead control.
...
- **The toolbar named the account a chat would take next, not the one it is
  ...
  so a pin decides the next process and nothing about the one answering.
  Reporting the pin made the toolbar say Work while `CLAUDE_CONFIG_DIR` in that
  very chat said personal. The router records the account it hands each agent as
  it starts it, kept apart from the pin because a pin is a different fact, and
  that is what the label names. Where the two differ the panel says which account
  the conversation is on and which it will come up on next.

- **The panel and the toolbar disagreed in plain sight.** Level one showed the
  workspace route while the toolbar showed the chat's account, so where a chat
  carried a pin the panel said Personal and the toolbar beside it said Work. Both
  were right about different questions. Level one reads what the toolbar reads.

- **An open panel described the place it was opened from.** Moving to another
  workspace left `This chat in macau` above a window showing amman. It is redrawn
  with the toolbar when the workspace or chat changes.

- **The toolbar kept the last chat's account after switching chats.** Each chat
  can be on its own account, so a leftover label does not read as stale, it names
  the wrong account. The button survives a switch, so nothing prompted a re-read
  and the old label stood until the panel was opened by hand.
...
- **Switching chats felt slow.** Two process spawns stood between the switch and
  ...
  account. The first is the same answer every time, so it is asked once and kept.
  The chat is watched on a short interval as well as on DOM mutation, because a
  switch need not touch the toolbar's own subtree.

- **Every lookup in Conductor's database could fail silently.** It is in WAL
  mode, and a WAL database opened read-only fails outright unless its `-shm` file
  already exists, which it does not while Conductor is closed or between a quit
  and the next launch. `sqlite3` exited non-zero with an empty result and the
  status was ignored, so an error and an empty table were the same value:
  `workspaces`, `repos`, `resolve` and the open chat all read as "Conductor knows
  of none". A refusal is retried as `immutable=1`, which reads the file without
  the shared index.
...
- **`no such directory` where an account should have been.** Conductor records a
  ...
  about one that does not exist on disk yet. Nothing in that answer needs the
  directory: routes are matched as paths.

- **The panel said "Workspace" while acting on a chat.** The scope line reads
  `This chat in <name>` when a chat is open, `The next workspace in <repo>` in
  the New Workspace view, and the note under the accounts says which of the two a
  choice writes.
